// Define topArtistsArray as global variable
let topArtistsArray = [];

// Constants
const CLIENT_ID = 'a4ebc67983d04b01877276535eeaffea';
...
function getUserProfileData(accessToken) {
    ...
    .then(data => {
        ...
        getUserTopArtistsShort(accessToken);
...
function downloadUserData() {
    ...
    if (storedData) {
        ...
        link.href = url;
        link.download = 'userProfileDataShortArtists.json'; // Set the file name with .json extension
...
let topArtistsArrayShort = []; // Define topArtistsArrayShort globally

// Function to handle fetching Top Artist Short data
function getUserTopArtistsShort(accessToken) {
    // Make a GET request to the Spotify API endpoint for user profile
    ...
    .then(data => {
        ...
        const topArtistsShortTemp = data.items.map(item => {
            let imageProperties = [];
            ...
            if (imageProperties.length > 0) {
                ...
                trackObject.tArtistsTime = 'Short_term'; // Include the time range here
                return trackObject;
            }
        });

        // Assign the array to the global variable
        topArtistsArrayShort = topArtistsShortTemp;

        // Convert the array to a JSON string and save to local storage
        var jsonString = JSON.stringify(topArtistsArrayShort);
        
        // You can save the JSON string to local storage
        localStorage.setItem('topArtistsArrayShort', jsonString);

        console.log(topArtistsArrayShort);

    })
    .catch(error => console.error(`Error fetching user top artists Short:`, error));
}
        
// Function to download user Top Artists Short data as a JSON file
function downloadTopArtistsShort() {

// Retrieve the JSON string from localStorage
var storedData = localStorage.getItem('topArtistsArrayShort');
...
var link = document.createElement('a');
link.href = url;
link.download = 'topArtistsShort.json'; // Set the file name with .json extension
...
console.log('No user artists Short data found in localStorage');
}
}

// Add an event listener to a link element to trigger the download function on click
document.getElementById('downloadArtistsShortLink').addEventListener('click', function(event) {
// Prevent the default action of the link
event.preventDefault();

// Call the function to download user playlists data
downloadTopArtistsShort();
});